...
class OTAUpdater:
    """
    A class to update your MicroController with the latest version from http
    optimized for low power usage.
    """

    def __init__(self, release_url, module='', main_dir='app', new_version_dir='next', secrets_file=None, headers={}):
    ...
    def get_latest_version(self):
        latest_release = self.http_client.get('{}/latest'.format(self.release_url))
        gh_json = latest_release.jsontext()
        print(gh_json)
        try:
            ...
            raise ValueError(
                "Release not found: \n",
                "Please ensure 'tag_name' is in the 'latest' file\n",
                "Now the latest json is: \n {} \n ".format(gh_json)
            )
    ...
    def _download_all_files(self, version, sub_dir=''):
        url = '{}/contents{}{}{}'.format(self.release_url, self.main_dir, sub_dir, version)
        print(url)
        gc.collect() 
        file_list = self.http_client.get(url)
        file_list_json = file_list.jsontext()
        print(file_list_json)
        for file in file_list_json:
            path = self.modulepath(self.new_version_dir + '/' + file['path'].replace(self.main_dir + '/', ''))
            if file['type'] == 'file':
                remotePath = file['path']
                print('\tDownloading: ', remotePath, 'to', path)
                self._download_file(version, remotePath, path)
            elif file['type'] == 'dir':
    ...
    def _download_file(self, version, remotePath, path):
#         print('https://raw.githubusercontent.com/{}/{}/{}'.format(self.github_repo, version, gitPath))
#         print('https://raw.sevencdn.com/{}/{}/{}'.format(self.github_repo, version, gitPath))
#         self.http_client.get('https://raw.githubusercontent.com/{}/{}/{}'.format(self.github_repo, version, gitPath), saveToFile=path)
        self.http_client.get('{}/{}'.format(self.release_url, remotePath), saveToFile=path)
